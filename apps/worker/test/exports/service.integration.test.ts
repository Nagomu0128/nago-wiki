...
describe("portable export service", () => {
  ...
  it("selects annual retention for the first weekly backup of a month", async () => {
    const create = vi.fn(() => Promise.resolve({ id: "workflow" }));
    const environment = {
      ...env,
    ...
    expect(result).toMatchObject({ started: 1, resumed: 0 });
    expect(
      await env.DB.prepare(
        "SELECT backup_date, retention_class FROM exports WHERE purpose = 'backup'",
      ).first(),
    ).toEqual({ backup_date: "2026-08-02", retention_class: "monthly" });
  });

  it("removes all expired artifact objects before cancelling the job", async () => {
    ...
    await env.DB.prepare(
      `INSERT INTO exports (
         ...
         status, r2_key, plan_r2_key, created_at, updated_at, expires_at
       ) VALUES (
